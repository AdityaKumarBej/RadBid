...
export default function Dashboard(props) {
	...
	useEffect(async () => {
		...
		let username = profile.entityName;
	...
	const populatePageData =  async (reqConfig) => {
		...
		for (var i = 0; i < open_tasks.length; i++) {
			...
			let parsedObject = (open_tasks[i].description);
		...
		setTotalActionsData(
			{
				...
				title_2: `${percentage_increase}%`,
				sub_title: 'Total bids have taken place in RadBid since last week.',
                content: '546K$ total price of all the items sold at the auction',
		...
		setContactsData([
			{
				...
				sub_title: 'New Users since yesterday',
				content: 'These users have been registered by you.',
			},
			{
				id: '2',
				...
				content:
					`Your list of users has ${total_contacts} contacts in Idle status. Why don't try re-engaging with them.`,
			},
		]);
		...
		if (totalContacts > 0) {
			...
			let lieutanant_contacts = contactHistoryObj[0].lieutanant;
			let captain_contacts = contactHistoryObj[0].captain;
			let colonel_contacts = contactHistoryObj[0].colonel;
			let major_contacts = contactHistoryObj[0].major;

			let tempArr = [];

			let p1 = Math.round((Number(lieutanant_contacts) / Number(totalContacts) * 100), 2);
			let p2 = Math.round((Number(captain_contacts) / Number(totalContacts) * 100), 2);
			let p3 = Math.round((Number(colonel_contacts) / Number(totalContacts) * 100), 2);
			let p4 = Math.round((Number(major_contacts) / Number(totalContacts) * 100), 2);
			...
			setEngagementBreakdown([
				{ id: '1', Name: 'Lieutenant', Value: `${p1}%` },
				{ id: '2', Name: 'Captain', Value: `${p2}%` },
				{ id: '3', Name: 'Colonel', Value: `${p3}%` },
                { id: '4', Name: 'Major', Value: `${p4}%` },
			]);
		} else {
		...
		if (dictionary.length > 0) {
			...
			for (var i = 0; i < dictionary.length; i++) {
				...
				for (var j = dictionary[i].Data.length - 1; j >= 0; j--) {
                    ...
                    console.log("CONTACTS ARRAY", contacts_uid_array);
					...
					let contact_full_name = dictionary[i].Data[j].contactsFirstName[0] + " " + dictionary[i].Data[j].contactsLastName[0] + " " + dictionary[i].Data[j].contactsLastName[0]
					...
					temp_Data_obj["mapContact"].push({ campaign_id: campaign_id, campaign_instance_id: campaign_instance_id, campaign_name: auction_name })
					...
					if (total_number_of_contacts_grouped_in_this_activity === 1) {
						...
						if (total_number_of_contacts_grouped_in_this_activity === 2) {
                            console.log("checking this", first_contact_fullname)
                            console.log("checkthing this", first_contact_name)
							temp_Data_obj["ActivityAvatar"] =
								[
									{
						...
						for (var c = 3; c < contacts_uid_array.length; c++) {   //index of 3 will have the 4th contact details
							let name_of_contact_obj = contact_info_obj_arr[c].firstname + " " + contact_info_obj_arr[c].middleinitial +  "" + contact_info_obj_arr[c].lastname 
                            //  await api.getContactInfo(contacts_uid_array[c], "false", reqConfig);
							...
							<div className={styles.WelcomeBlockStyles}>
								...
								<div className={styles.actionButtons}>
									...
									<button className={Trigger ? styles.backdrop : styles.PendingActionButtonStyles} onClick={() => handleAction(true)} >
										<span className={styles.PendingActionButtonSpan1}>{pendingActionsCount}</span>  Pending Actions    &#62;
									</button>
								</div>
							</div>
							<h1 className={styles.welcomtextstyle}>
								{current_time} {(username).charAt(0).toUpperCase() + (username).slice(1)}
							</h1>
						</div>
					</Container>
					...
					<Container>
						<Grid>
							<div className={styles.userActivityStyles}>
								...
								<div
								//className={styles.seeAllActivityButton}
								// style={{ color: '#00578E', float: 'right' }}
								>
								...
								<div className={styles.card} style={{ "marginLeft": '1em', 'height': '335px' }}>
									...
									<div className='card-body' style={{ 'lineHeight': '8em' }}>
										...
										<h5 style={{
											...
											Total bids have taken place in RadBid since last week.</h5>
								...
								<div style={{ display: 'flex', flexWrap: 'wrap', gap: 20 }}>
									...
									<Card sx={{ width: 271, height: 110 }} style={{ borderRadius: 15 }}>
										<CardContent style={{ padding: '6%' }}>
											...
											>
												<img src='/paypal.png' style={{ width: 54 }} />
											</Box>
											<Typography
												variant='h5'
												sx={{
						...
						<div className={styles.ContactsSectionStyles}>
...

							{
								(engagementDataAvailable)
									...
									<ol>
										{' '}
										<h5>User Entitlements Breakdown</h5>
